...
def main(args):
    ...
    model.save(os.path.join(args.output_folder, f"model_{num_of_samples}.keras"))
    with open(os.path.join(args.output_folder, f"history_{num_of_samples}.pkl"), "wb") as f:
        pkl.dump(history, f)
    evaluation_results = ai.evaluate_model(model, test)
    with open(os.path.join(args.output_folder, f"results_{num_of_samples}.pkl"), "wb") as f:
